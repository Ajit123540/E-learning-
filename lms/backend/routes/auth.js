...
router.post('/register', async (req, res) => {
  ...
  try {
    const { db } = req;
    console.log('Database users:', db.data.users);
...
router.post('/login', async (req, res) => {
  ...
  try {
    ...
    // Check if user exists by username (name) or email
    const user = db.data.users.find(u => u.name === loginIdentifier || u.email === loginIdentifier);
    if (!user) {
